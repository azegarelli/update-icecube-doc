...
where $N_{\mathrm{signal}}$ and $N_{\mathrm{background}}$ are the expected numbers of signal and background events (both atmospheric neutrinos and muons) with reconstructed event energy $E$ above the selected energy proxy $E_{\mathrm{event}}$ at declination $\delta$, derived from Monte Carlo simulations. **It represents the probability that an event with $E > E_{\mathrm{event}}$ at declination $\delta$ is of astrophysical origin.**
...
- `ra_dec_error` - Combined positional uncertainty at 90% containment. For `"record_number"`=`1`, it is obtained by combining the asymmetric uncertainties in RA and Dec, as reported in the GCN Circular, accounting for the spherical geometry of the sky. In particular, the RA uncertainty is rescaled by a factor \cos(\delta), since lines of constant RA converge toward the poles. First, we define symmetric uncertainties as the average of the upper and lower bounds:
```python
ra_err_mean = (abs(ra_err_plus) + abs(ra_err_minus)) / 2
dec_err_mean = (abs(dec_err_plus) + abs(dec_err_minus)) / 2
```
The combined positional uncertainty is then defined as:
```python
err_combined = np.sqrt(ra_err_mean*np.cos(dec)*dec_err_mean)
```
This expression corresponds to the radius of a circle having the same area as the error ellipse defined by the projected RA and Dec uncertainties. In this sense, it provides an effective scalar representation of the positional uncertainty that preserves the area of the confidence region. The quantities `ra_err_plus`, `ra_err_minus`, `dec_err_plus`, and `dec_err_minus` correspond to the 90% containment uncertainties reported in the GCN circular associated with the event.
...
The IceCube alert archive is publicly accessible at: [IceCube Alert Archive](https://roc-2.icecube.wisc.edu/public/alerts/).
